...
export type WorktreeSubcommand = "add" | "list" | "open" | "close" | "refresh" | "remove" | "merge" | "send" | "prune" | "restore" | "archive" | "unarchive" | "label" | "profile" | "tab" | "reorder";
...
async function listWorktrees(
  ...
  const savedOrder = (await readWorktreeOrderState(projectGitDir)).branches;
  const orderComparator = buildWorktreeOrderComparator(savedOrder);
  ...
  const matchingRows = rows
    ...
    .sort((a, b) =>
      savedOrder.length > 0
        ? orderComparator(a.branch, b.branch)
...
export async function runWorktreeCommand(
  ...
  try {
    ...
    const command: Exclude<WorktreeSubcommand, "add" | "list" | "send" | "prune" | "restore" | "label" | "profile" | "tab" | "reorder"> = context.command;
