...
const BASH_SCRIPT = `_webmux() {
  ...
  if [[ \${COMP_CWORD} -eq 1 ]]; then
    COMPREPLY=($(compgen -W "serve init service update add oneshot list open close refresh archive unarchive label profile remove merge send prune linear project completion" -- "\${cur}"))
    return
  fi
