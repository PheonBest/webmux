...
/** opencode has no JSON hook-config file like Claude/Codex — lifecycle events are
 *  wired up via its TypeScript plugin system instead (`.opencode/plugin/*.ts`,
 *  auto-loaded from the project directory; see
 *  https://opencode.ai/docs/plugins/). The plugin shells out to the same
 *  `webmux-agentctl` script the Claude/Codex hook commands call, so all three
 *  agents report lifecycle events through one control-plane contract. Confirmed
 *  hook names: `event` (catch-all bus subscription), `tool.execute.before`,
 *  `tool.execute.after`. There is no confirmed dedicated "user submitted a
 *  prompt" hook, so `tool.execute.before` (and the `event` catch-all opening a
 *  new turn) doubles as the "running" signal — slightly coarser than Claude's
 *  UserPromptSubmit hook.
 */
function buildOpencodePlugin(input: { agentCtlPath: string }): string {
  const agentCtl = JSON.stringify(input.agentCtlPath);
  return `// Generated by webmux — do not edit by hand, see backend/src/adapters/agent-runtime.ts
import { spawn } from "node:child_process";

const AGENT_CTL = ${agentCtl};

function runAgentCtl(args) {
  try {
    const child = spawn(AGENT_CTL, args, { stdio: "ignore", detached: true });
    child.unref();
  } catch {
    // best-effort — lifecycle reporting should never break the agent run
  }
}

function findPrUrl(value, seen = new Set()) {
  ...
  if (value && typeof value === "object" && !seen.has(value)) {
    ...
    for (const child of Object.values(value)) {
      const found = findPrUrl(child, seen);
...
export const WebmuxLifecyclePlugin = async () => {
  return {
    event: async ({ event }) => {
      if (event?.type === "session.idle") {
        runAgentCtl(["status-changed", "--lifecycle", "idle"]);
      } else if (event?.type === "session.error") {
        const message = typeof event?.properties?.error === "string"
          ? event.properties.error
          : "opencode session error";
        runAgentCtl(["runtime-error", "--message", message]);
      }
    },
    "tool.execute.before": async () => {
      runAgentCtl(["status-changed", "--lifecycle", "running", "--best-effort"]);
    },
    "tool.execute.after": async (input, output) => {
      if (input?.tool !== "bash" || !output) return;
      const command = output?.args?.command ?? output?.input?.command;
      if (typeof command !== "string" || !command.includes("gh pr create")) return;
      const url = findPrUrl(output);
      const args = ["pr-opened"];
...
export default WebmuxLifecyclePlugin;
`;
}

function mergeOpencodePluginRegistration(existing: Record<string, unknown>, relativePluginPath: string): Record<string, unknown> {
  const existingPlugins = Array.isArray(existing.plugin) ? existing.plugin.filter((entry) => typeof entry === "string") : [];
  const plugins = existingPlugins.includes(relativePluginPath)
    ? existingPlugins
    : [...existingPlugins, relativePluginPath];
  return { ...existing, plugin: plugins };
}

async function ensureOpencodePluginRegistered(configPath: string, relativePluginPath: string): Promise<void> {
  ...
  const merged = mergeOpencodePluginRegistration(existing, relativePluginPath);
...
export async function ensureAgentRuntimeArtifacts(input: {
  ...
  const artifacts: AgentRuntimeArtifacts = {
    ...
    opencodePluginPath: join(input.worktreePath, ".opencode", "plugin", "webmux.ts"),
    opencodeConfigPath: join(input.worktreePath, "opencode.json"),
  };
  ...
  await mkdir(dirname(artifacts.opencodePluginPath), { recursive: true });
  ...
  await Bun.write(artifacts.opencodePluginPath, buildOpencodePlugin({ agentCtlPath: artifacts.agentCtlPath }));
  await ensureOpencodePluginRegistered(artifacts.opencodeConfigPath, "./.opencode/plugin/webmux.ts");
